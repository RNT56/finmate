...
>
> **Explicitly still open** (the substantive runtime/submission gaps — release infra is wired, the product behind it is not yet live): the **live Supabase-backed data layer** — `supabase-swift` / `supabase-js` repository implementations behind the `Domain`/`core` protocols are still **in-memory** across all pillars (the schema migrations exist in `supabase/migrations/` but are not yet wired); the **live `market-data` Edge Function** wiring (sample rates today — M5-BACK-01, M5-CALC-02); the **biometric-lock runtime** (`LocalAuthentication` gating/timeout — the toggle persists intent only, M8-SEC-04); the **real data-export + `delete-account`** calls (Settings stubs today, M7-SET-03, M8-STORE-04); the **DB/RLS pgTAP regression gate actually running** (the CI workflows do **not** yet include it — it needs Docker / a Supabase project, M0-CI-06); **TestFlight / App Store submission itself** (privacy nutrition label, screenshots/metadata, live legal URLs, the external beta — M8-STORE-03/05/07/08, and the actual run of M8-STORE-01); and the **remainder of M7** (dashboard M7-HOME, performance M7-PERF, the full accessibility audit M7-A11Y, the Supabase `PreferencesRepository` M7-SET-01).
