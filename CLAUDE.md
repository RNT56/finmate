...
**Greenfield → in build. Docs complete; backend deployment-ready; M0 foundation + M1–M6 (Subscriptions, Income & Expenses, money-flow, payday calendar, assets/crypto/multi-currency, CSV import) + M7 (partial): Settings + appearance + a11y polish + customizable Home dashboard + M8 (partial): App Store Info.plist export/Face-ID hardening built, compiled, and tested on both clients.**
...
- [x] **M0–M6 (iOS) executed & verified** — `Packages/FinmateCore` (`Domain` + `Shared`) implements the [docs/13](./docs/13-algorithms-and-calculations.md) algorithms (incl. §6 cash-flow metrics) with passing unit tests (`swift test`); the iOS app (`App/`, generated from `project.yml` via XcodeGen) **builds for the iPhone 17 / iOS 26 simulator** and boots — Liquid Glass UI, root `TabView`, a Subscriptions slice (list + add, a **detail** screen, a **Swift Charts** category-distribution analytic, delete/reorder), and a live **Cash Flow** tab — **Income & Expenses + cash-flow metrics (M2)** with the **Monthly Income / Monthly Expenses / Net / Savings-rate %** KPIs computed by the Domain `CashFlowMetrics` math. **M3 (cost-tracker money-flow) shipped** — the bucketed money-flow **value model** per [ADR-0016](./docs/12-decisions-adr.md) (`MoneyFlow`: income + Fixed/Variable/Subscriptions buckets, `totalExpensesMinor`, the derived `savingsMinor = max(0, income − expenses)` clamp, [docs/13](./docs/13-algorithms-and-calculations.md) §6.5) is in the shared core (Swift `Domain` + web `core/`, unit-tested with matching vectors), rendered as a `Canvas`/`Path` Sankey on iOS and inline SVG on web, and wired into the Cash Flow screen on **both** clients (Income → Fixed/Variable/Subscriptions/Savings with proportional ribbons). **M4 (payday calendar) shipped** — a pure recurrence engine (`PaydayCalendar` / `recurrence.ts`: subscription charges, income paydays, fixed-expense due dates with anchor-based day-of-month clamping, [docs/13](./docs/13-algorithms-and-calculations.md) §11) in the shared core with matching Swift+TS vectors, surfaced as a month-calendar with event dots + day detail on **both** clients, plus an iOS `UNUserNotificationCenter` reminder scheduler (ADR-0013, gated). **M5 (assets, crypto calculator, multi-currency) shipped** — `FinancialAsset`/`AssetTransaction` + average-cost valuation (ADR-0015: unrealized gain/loss, portfolio total, allocation distribution), a `CurrencyConverter`-backed BTC/sats calculator, and an EUR/USD/BTC display-currency switcher, surfaced as an Assets screen (allocation donut) + a Calculator under a **More** hub on **both** clients — using sample exchange rates pending the live `market-data` Edge Function wiring. **Income, fixed/variable expenses, and assets are fully editable** on both clients (add/edit/delete forms + buy/sell/dividend asset transactions with average-cost recompute), wired through the repository protocols. **Auth & onboarding shipped** on both clients (`M0-AUTH-*`) — an `AuthRepository` protocol + a Keychain-backed `SupabaseAuthRepository` (Sign in with Apple via `signInWithIdToken`/`signInWithOAuth`, email/password sign-in/sign-up, `signOut`) and a `DemoAuthRepository`; an Auth screen (Sign in with Apple + email/password + a **Try-the-demo** offline path), first-run onboarding (currency/appearance/biometric), session-driven routing (`AuthStore`), and logout. The default offline build boots to the Auth screen and the demo path keeps the app fully usable; **live sign-in runs against Supabase only when the project is configured** (runtime deferred). **M6 (CSV import) shipped** — a pure RFC-4180-lite importer (tokenizer + header aliases + per-row validation + valid/errors **preview before write**, [docs/13](./docs/13-algorithms-and-calculations.md) §9) in the shared core (`SubscriptionCSVImporter` / `csvImport.ts`, unit-tested), with an Import screen (paste/file → column-mapping → preview → import valid rows) under the **More** hub on **both** clients; import now covers **all four entity types** (subscriptions, income, fixed/variable expenses) via a shared `CSVImportKit` + per-type importers + a "What are you importing?" type selector (expense rows resolve a category name → id). **M7 (partial — Settings + app-wide appearance + an a11y/Dynamic-Type polish pass) shipped** — the canonical `UserPreferences` value type + `PreferencesRepository` protocol + an in-memory impl live in the shared core (Swift `Domain` `UserPreferences.swift`, web `core/preferences.ts`; `Appearance {system,light,dark}`, default currency, biometric-lock + reminder toggles, clamped 0…30 lead-time), unit-tested for defaults/clamp/round-trip; the **More** hub's Settings stub became a real screen — iOS `SettingsView` + `PreferencesStore` (`App/Sources/Settings.swift`) and web `Settings.tsx` + `usePreferences`/`PreferencesProvider` — with **appearance applied app-wide** (iOS `.preferredColorScheme`; web `data-theme` on `<html>` honoring `prefers-color-scheme` for `system`) and VoiceOver/`role`/`aria` accessibility polish. **The customizable Home dashboard (M7-HOME) also shipped** — a **card registry** of analytics-backed cards (subscriptions monthly total, cash-flow KPIs, assets portfolio value/gain, upcoming charges via `PaydayCalendar`) that the user can **reorder** and **show/hide**, with the resulting **`card_order`** persisted behind a dashboard-layout repository on **both** clients (iOS `HomeView`/`DashboardStore`; web `Home.tsx` + `useDashboard`) in the single Liquid Glass language; persistence is in-memory (iOS) / `localStorage` (web) today, with the canonical `dashboard_layouts.card_order` table ([docs/05](./docs/05-data-model.md) §3.11) as the eventual Supabase-backed store. The **a11y / Dynamic-Type pass is complete** (M7-A11Y-01/02/03/04): scalable type proven at `AccessibilityXXL` (iOS UI test) + 200%-zoom no-overflow (web Playwright), WCAG-AA contrast, full reduce-motion gating, and a VoiceOver labels/hints/traits sweep over every interactive control. The biometric-lock **runtime** (`LocalAuthentication` app-lock gate) and in-app **data export** (JSON bundle, money as raw minor units) have shipped. **Tests cover the full pyramid** — pure logic + the `@Observable` store / React-hook layer + iOS XCUITest (incl. AccessibilityXXL) + web Playwright E2E (smoke + large-text + CSV-import + currency-switch), with CI coverage gates on both clients. The rest of M7 (performance budgets) stays open. **M8 (partial — CI/CD + Fastlane + lint + App Store Info.plist hardening) shipped** — the release-engineering scaffold is in-repo: **CI workflows** (`.github/workflows/ios.yml` XcodeGen + `swift test` + `xcodebuild` iPhone 17, `web.yml` lint/build/test, `security.yml` Gitleaks), **Fastlane** `test`/`beta` lanes + signing config (`fastlane/Fastfile`/`Appfile`: App Store Connect API key + `fastlane match` readonly-on-CI, bundle id `app.finmate.Finmate`, secrets never committed), **SwiftLint** (`.swiftlint.yml`) + web **ESLint flat config + Prettier** (`npm run lint` passes), and the **App Store Info.plist keys** — `project.yml` declares `INFOPLIST_KEY_ITSAppUsesNonExemptEncryption = NO` (export-compliance exempt) and `INFOPLIST_KEY_NSFaceIDUsageDescription` (Face ID usage string for the biometric-lock toggle), both **verified present in the built `Finmate.app/Info.plist`** after `xcodegen generate` + `xcodebuild` ([docs/07](./docs/07-security-and-privacy.md) §9.2, [docs/10](./docs/10-task-backlog.md) M8-STORE-01/02/06). This is release *infra* only — the live Supabase data layer, signing-secret provisioning, the DB/RLS regression gate, and the submission itself remain open (see below). Both clients verified GREEN (latest): `swift test` (**192** Domain) + `xcodebuild … iPhone 17` **BUILD SUCCEEDED** + `xcodebuild test` (**18** store + **4** XCUITest incl. AccessibilityXXL) **TEST SUCCEEDED**; web `npm run lint` + Vitest (**285**) + `npm run build` + Playwright e2e (**8**: smoke + large-text + critical-flows).
- [x] **Web client scaffolded & verified** ([ADR-0021](./docs/12-decisions-adr.md)) — a separate Vite + React 19 + TypeScript + `supabase-js` app under `web/`; **builds** (`npm --prefix web run build`) and previews the Home + Subscriptions + **Cash Flow** slices in the Liquid Glass language (CSS `backdrop-filter`), with passing Vitest tests whose TS `core/` mirrors the Swift `Domain` and shares [docs/13](./docs/13-algorithms-and-calculations.md)'s test vectors. The web **Cash Flow** section ships the same **M2** KPIs (Monthly Income / Monthly Expenses / Net / Savings-rate %) **plus the M3 money-flow Sankey** (SVG) and the **M4 payday calendar** (month grid + the shared recurrence engine), and the **M5 Assets / Calculator / More** screens (portfolio €27,250 / +€5,250, allocation donut, BTC calculator, EUR/USD/BTC switcher), plus **M6 CSV import** (validate + preview), and the **M7 (partial) Settings** surface (`Settings.tsx` + a `PreferencesProvider` that applies appearance app-wide via a `<html>` `data-theme`), so iOS and web agree on the figures. Architecture & plan: [docs/16-web-client.md](./docs/16-web-client.md). iOS remains the lead client.
- [x] **Live Supabase data layer wired (compile-verified; offline fallback).** Supabase-backed repositories now sit behind the `Domain`/`core` protocols on **both** clients: iOS `Packages/FinmateData` (a `DataLayer` SPM package on `supabase-swift` 2.48 — Keychain `AuthLocalStorage`, `Supabase{Subscription,Income,Expense,Asset,Preferences,Category}Repository` with snake_case↔camelCase + `Int64` mappers and the `delete_subscription`/`batch_reorder_subscriptions` RPCs, plus a `MarketDataRateProvider` over `functions.invoke("market-data")`); web `features/*/supabaseRepository.ts` + `lib/repositories.ts` over `supabase-js`. A config-driven selector uses Supabase when `SUPABASE_URL` + anon key are set and the in-memory sample repos otherwise, so both clients still build and preview offline (`FinmateCore` stays dependency-free — 98 tests; `xcodebuild` **BUILD SUCCEEDED**; web `npm run lint` + build + **133 tests**). **Compile-verified, NOT yet runtime-tested** (no hosted project here).
- [ ] **Remaining build:** **runtime verification of the data layer against a hosted project** (live RLS round-trips, the `market-data` Edge Function, the `delete_subscription`/`batch_reorder_subscriptions` RPCs); **all the DTO↔schema divergences flagged during wiring are now resolved** — the **asset-type enum** and the **weekly fixed-expense** by [ADR-0023](./docs/12-decisions-adr.md) (migration `20260628001000_reconcile_enums.sql` widens both `CHECK`s to the union set, the Domain `AssetType` gains `savings` + `realEstate`/`real_estate`, and the asset DTO round-trips losslessly 1:1), and the expense `category_id` divergence by [ADR-0022](./docs/12-decisions-adr.md) (`FixedExpense`/`VariableExpense` now carry `categoryID`/`categoryId` with names resolved client-side from the categories list, matching `Subscription` + schema); the **live `market-data` rates in the UI** (clients use sample exchange rates today); the real **delete-account** call (stub today; the biometric-lock runtime + data export have shipped); **hosted deploy** (needs the owner's Supabase account) + local `supabase db reset` apply + the **DB/RLS pgTAP regression gate** in CI (need Docker / a project); signing-**secret provisioning** (ASC API key + private `match` repo + the GitHub secrets) and **TestFlight / App Store submission**; and the **remainder of M7** — performance budgets, the full accessibility audit, and the `user_preferences` migration (the customizable Home dashboard has since shipped on both clients — `card_order` persisted via the in-memory/`localStorage` repo pending the Supabase-backed `dashboard_layouts` store).
