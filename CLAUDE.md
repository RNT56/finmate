...
**Greenfield → in build. Docs complete; backend deployment-ready; M0 foundation + M1–M6 (Subscriptions, Income & Expenses, money-flow, payday calendar, assets/crypto/multi-currency, CSV import) + M7 (partial): Settings + appearance + a11y polish built, compiled, and tested on both clients.**
...
- [ ] **Remaining build:** **M7 (remainder)** — the customizable/reorderable dashboard, performance-budget work, the full accessibility audit (beyond this polish pass), the biometric-lock runtime (the Settings toggle persists intent only), and the `user_preferences` migration + Supabase-backed `PreferencesRepository` — plus **M8** (full UIs/hardening on **both** iOS and web), the live `market-data` Edge Function wiring (clients use sample exchange rates today), the real Supabase-backed data layer (swap the in-memory repos for `supabase-swift` / `supabase-js` against RLS), local DB apply (`supabase db reset`, needs Docker running), and hosted deploy (needs the owner's Supabase account).
