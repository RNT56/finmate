...
- [ ] **Remaining build:** **runtime verification of the data layer against a hosted project** (live RLS round-trips, the `market-data` Edge Function, the `delete_subscription`/`batch_reorder_subscriptions` RPCs) + reconciling the DTO↔schema divergences flagged during wiring (asset-type enum, expense `category_id` vs free-text, weekly fixed-expense — fix task spawned); the **live `market-data` rates in the UI** (clients use sample exchange rates today); the **biometric-lock runtime** (`LocalAuthentication` gating/timeout — the Settings toggle persists intent only) and the real **delete-account** / data-export calls (stubs today); **hosted deploy** (needs the owner's Supabase account) + local `supabase db reset` apply + the **DB/RLS pgTAP regression gate** in CI (need Docker / a project); signing-**secret provisioning** (ASC API key + private `match` repo + the GitHub secrets) and **TestFlight / App Store submission**; and the **remainder of M7** — performance budgets, the full accessibility audit, and the `user_preferences` migration (the customizable Home dashboard has since shipped on both clients — `card_order` persisted via the in-memory/`localStorage` repo pending the Supabase-backed `dashboard_layouts` store).
