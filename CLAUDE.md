...
- [ ] **Remaining build:** the **live Supabase-backed data layer** (swap the in-memory repos for `supabase-swift` / `supabase-js` repository implementations behind the `Domain`/`core` protocols, against RLS — the migrations exist in `supabase/migrations/` but are not yet wired); the **live `market-data` Edge Function wiring** (clients use sample exchange rates today); the **biometric-lock runtime** (`LocalAuthentication` gating/timeout — the Settings toggle persists intent only); **hosted deploy** (needs the owner's Supabase account) + `supabase db reset` local apply + the **DB/RLS pgTAP regression gate** actually running in CI (needs Docker / a project — the workflows don't yet include it); signing-**secret provisioning** (the ASC API key + private `match` repo + the M0-CI-09 GitHub secrets, so the Fastlane lanes can run end-to-end); **TestFlight / App Store submission** itself (privacy nutrition label, screenshots/metadata, live legal URLs, external beta); and the **remainder of M7** — the customizable/reorderable dashboard, performance-budget work, the full accessibility audit (beyond the polish pass), and the `user_preferences` migration + Supabase-backed `PreferencesRepository`.
